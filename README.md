...
- Each level uses a distinct maze layout.
- Add `?diagnostics=1` to the URL to reveal the lower-right level dropdown for diagnostic testing.
...
PacBecca uses full `MAJOR.MINOR.PATCH` project versioning. The current release is shown in the top-right game badge as `v0.6.0`.
